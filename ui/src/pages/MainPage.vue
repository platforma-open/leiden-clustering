...
function setInput(inputRef?: PlRef) {
  ...
  if (inputRef)
    app.model.args.title = app.model.outputs.embeddingOptions?.find((o) =>
...
const graphState = computed({
  get: () =>
    data.currentTab === "umap" ? app.model.ui.graphStateUMAP : app.model.ui.graphStateTSNE,
  set: (value) => {
    if (data.currentTab === "umap") app.model.ui.graphStateUMAP = value;
    else app.model.ui.graphStateTSNE = value;
  },
});

const pFrame = computed(() =>
  data.currentTab === "umap" ? app.model.outputs.UMAPPf : app.model.outputs.tSNEPf,
);
    ...
    >
      ...
      <template #settingsSlot>
          ...
          >
            <template #tooltip>
              <div>
                ...
                <strong>Effect:</strong> Higher resolution creates more granular clusters, lower
                resolution creates broader clusters.
              </div>
            </template>
          </PlNumberField>
        </PlRow>
        ...
        <PlAlert v-if="app.model.args.resolution > 1" type="warn" :style="{ width: '320px' }">
          ...
          The selected resolution ({{ app.model.args.resolution }}) is above the recommended range
          (0.4-1.0). This may result in over-clustering with too many small clusters.
        </PlAlert>
        <PlAlert v-if="app.model.args.resolution < 0.4" type="warn" :style="{ width: '320px' }">
          <template #title>Low Resolution</template>
          The selected resolution ({{ app.model.args.resolution }}) is below the recommended range
          (0.4-1.0). This may result in under-clustering with too few large clusters.
        </PlAlert>
      </template>
    </GraphMaker>
  </PlBlockPage>
